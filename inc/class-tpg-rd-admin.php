...
class tpg_rd_admin {
	...
	public $rd=NULL;            //class
	public $rd_opts=array();
	public $rd_paths=array();
	public $plugin_data=array();
	public $module_data=array();

	function __construct($opts,$paths,$_rd) {
		$this->rd= $_rd;
		$this->rd_opts=$opts;
		$this->rd_paths=$paths;
	...
	*/
	...
	public function tpg_rd_show_settings() {
		...
		if( isset($_POST['rd_opts']) ) {
			if ( ! isset( $_POST['rd_wpnonce'] )
    	    || ! wp_verify_nonce( $_POST['rd_wpnonce'], 'rd_updt_nonce' )
			) {
				...
				$this->rd_opts=$this->rd->get_options();
			}
		...
		$page_content = str_replace("{icon}",'',$page_content);
				...
				<div class="inside"  style="padding:10px;">
					<form name="redirect_options" method="post" action="{action-link}">
						...
						<table class="form-table">
							...
							{$rdnonce('rd_updt_nonce', 'rd_wpnonce')}
							&nbsp;&nbsp;
